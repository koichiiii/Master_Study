...
def floor_control(floor,outside_temp):
    for room in floor.rooms.values():
        ...
        aircon_temp = calculate_temperature(room.aircons,room.temp_map, room.persons ,outside_temp)
                
        #エアコンのループ    
        for aircon, temp in aircon_temp:
            aircon.operate_property("temperature",temp)
        room.operate_property("set_temp",aircon_temp[0][1])
...
def auto_aircon_control(building):
    while not stop_threads:
        ...
        for floor in building.floors.values():
            thread = threading.Thread(target=floor_control, args=(floor, building.outside_temp),daemon=True)
        ...
        print(f"自動エアコン制御を終了します。\n{elapsed_time}  を {csv_filename1} に保存しました。")
...
def navigate(building):
    ...
    while True:
        ...
        person = building.persons[f"Building:person{random_number}"]
        
        ctemp = person.get_property("comfort_temp")
        rtemp = person.current_room.get_property("set_temp")
        diff = abs(ctemp - rtemp)
        # print(f"{ctemp}-{rtemp}={diff}")
        comfort_position = [person.current_room.name,(1,1,1),diff]
        # print(comfort_position)
        if diff >= 1:
            for floor in building.floors.values():
                for room in floor.rooms.values():
                    difference = abs(room.get_property("set_temp") - ctemp) 
                    if difference == comfort_position[-1]:
                        comfort_position = comfort_position + [room.name,(1,1,1),difference]
                    elif difference < comfort_position[-1]:
                        comfort_position =  [room.name,(1,1,1),difference]
        ...
        print(f"ナビゲーションを終了します。\n{elapsed_time}  を {csv_filename2} に保存しました。")
...
def main():
    global stop_threads
    max_ac_threads = 3  # 通常時のエアコン制御の最大スレッド数
    #インスタンス生成
